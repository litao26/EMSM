		...
		$.ajax({
            ...
            success:function(objs){
                for(var i=0;i<objs.length;i++){
                    var obj = objs[i];
                    //  把obj 对象数据拼接成一个显示的div
                    var  divStr = "<div class='item'>"
                        +"<h3><b>&gt;</b><a href='bookList.do?parent_id="+obj.category_id+"'"
                        +"class='dot'> "+obj.name+"</a></h3>"
                        +"</div>";
        ...
        $.ajax({
            ...
            success:function(objs){
                for(var i=0;i<objs.length;i++){
                    var obj = objs[i];
                    //  把obj 对象数据拼接成一个显示的div
                    var  divStr = " <li>・<a  Target='_blank'  href='#'>"+obj.content+"</a></li>";
                    $(".book_news").append($(divStr));
                }
            },
            dataType:"json"
        });
        $.ajax({
            ...
            success:function(products){
                for(var i=0;i<products.length;i++){
                    ...
                    var divStr = "<li class='fore'><a href='#' target='_blank'>"+product.publishing+"</a></li>";
                    $(".book_product").append($(divStr));
	...
	<div id="left">
    	<div id="book_sort">
            ...
            <div class="sort_list">
                ...
                <div class='item'>
                	<h3><b>&gt;</b><a href='bookList.do?parent_id=${category.category_id}&category_id=${pageContext.request.getParameter("category_id")}&name=${category.name}' class='dot'>
							${category.name}</a></h3>
                </div>
				</c:forEach>
            </div>          
        ...
        <div id="publishers">
            ...
            <ul>
				<c:forEach items="${product}" var = "MyProduct">
            	<li class='fore'><a href='#' target="_blank">${MyProduct.publishing}</a></li>
				</c:forEach>
            </ul>
			</div>
        </div>
			...
			<div class="book_content">
				<ul class="book_news">
					<c:forEach items="${news}" var="news">
                     <li>・<a  Target="_blank"  href="#">${news.content}</a></li>
					</c:forEach>
				</ul>
			</div>
		</div>
